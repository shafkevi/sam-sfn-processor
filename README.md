...
#### Requirements
This repo requires you to have your MongoDB credentials stored in a Secrets Manager secret already in the Key/value format like below:
```env
MONGO_PASSWORD
MONGO_USERNAME
MONGO_DOMAIN
```
MONGO_DOMAIN is something like: `your_app.fdafda.mongodb.net`

You will need the ARN of this secret to deploy the template.

#### First time
```bash
npm install
sam build
sam deploy --guided --parameter-overrides SecretARN=ARN_OF_YOUR_SECRET
```

### Test the workflow
```bash
node test-pipeline.js SOME_RANDOM_STRING_FOR_UNIQUENESS
```

#### Subsequent deploys
```bash
sam build && sam deploy \
  --parameter-overrides SecretARN=ARN_OF_YOUR_SECRET \
  --no-confirm-changeset
```

### Architecture
The App Runner app is not deployed from this stack, but is in the architecture diagram as a demonstration of how you might interact with this workflow.
![Architecture](architecture.svg)


## Below is the boiler plate set of documentation

### Feel free to skip over this if you already know SAM, otherwise it's a useful read!

This project contains source code and supporting files for a serverless application that you can deploy with the SAM CLI. It includes the following files and folders:

- functions - Code for the application's Lambda functions to running basic processing steps.
- statemachines - Definition for the state machine that orchestrates the stock trading workflow.
- template.yaml - A template that defines the application's AWS resources.

This application creates a basic passthrough workflow which runs on a pre-defined schedule (note that the schedule is disabled by default to avoid incurring charges). It demonstrates the power of Step Functions to orchestrate Lambda functions and other AWS resources to form complex and robust workflows, coupled with event-driven development using Amazon EventBridge.
...
```bash
sam-sfn-processor$ sam logs -n LAMBDA_FUNCTION_NAME --stack-name sam-sfn-processor --tail
```
...
### Note - these do not work and were not updated from the template. However you certianly could update the json file listed above to make your own tests!
...
```bash
aws cloudformation delete-stack --stack-name sam-sfn-processor
```
